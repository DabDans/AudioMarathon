...
sys.path.append("/data/to/your/Audio_Longbench/Code/Qwen_2.5")
...
try:
    ...
    print("[Info] Qwen-Omni KV Press Pipeline loaded successfully")
except Exception as e:
    print(f"[Warning] Qwen-Omni KV Press Pipeline import failed: {e}")
...
def parse_args():
    ...
    p.add_argument("--json-file", type=str, default="/data/to/your/Dataset/SLUE/merged_audio_data.json")
    p.add_argument("--audio-base-dir", type=str, default="/data/to/your/Dataset/SLUE")
    p.add_argument("--max-new-tokens", type=int, default=10)
    p.add_argument("--min-seq-len", type=int, default=128)
    p.add_argument("--no-compress", action="store_true")
    p.add_argument("--use-pipeline", action="store_true", help="Use Qwen-Omni KV Press Pipeline")
    return p.parse_args()
        ...
        if hasattr(model, 'thinker') and hasattr(model.thinker, 'model'):
            ...
            if hasattr(base_model, 'layers') and len(base_model.layers) > 0:
                ...
                if hasattr(first_layer, 'self_attn'):
                    print("[KVPress] Multimodal compression method compatibility passed for Qwen2.5-Omni")
                    return True
        print("[KVPress] Compatibility verification failed: Model structure does not meet requirements")
        return False
    except Exception as e:
        print(f"[KVPress] Compatibility verification error: {e}")
        return False
...
def apply_kv_press_to_model(model, press_obj):
    """Apply KV Press to the model"""
    if press_obj is None or not KV_PRESS_AVAILABLE:
        return model
    try:
        if hasattr(model, 'thinker') and hasattr(model.thinker, 'model'):
            target_model = model.thinker.model
            print(f"[KVPress] Applying compression to thinker.model: {type(press_obj).__name__}")
        else:
            target_model = model
            print(f"[KVPress] Applying compression to base model: {type(press_obj).__name__}")
        press_obj.apply(target_model)
        print("[KVPress] Compression applied successfully")
        return model
    except Exception as e:
        print(f"[KVPress] Compression application failed: {e}")
        traceback.print_exc()
        return model


def patch_qwen_for_kvpress(model):
    """As in GTZAN: ensure base_model and each layer's self_attn have compatible rotary_emb"""
    try:
        base_model = None
        if hasattr(model, 'thinker') and hasattr(model.thinker, 'model'):
            base_model = model.thinker.model
...
def create_qwen_kvpress_pipeline(model, processor, press_obj):
    """Create Qwen-Omni KV Press Pipeline"""
    if not QWEN_KVPRESS_PIPELINE_AVAILABLE:
        print("[Warning] Qwen-Omni KV Press Pipeline not available, using legacy method")
        return None

    try:
        pipeline = QwenOmniKVPressAudioPipeline(model=model, processor=processor)
        print("[Info] Qwen-Omni KV Press Pipeline created successfully")
        return pipeline
    except Exception as e:
        print(f"[Warning] Pipeline creation failed: {e}")
        traceback.print_exc()
        return None


def process_sample_with_pipeline(pipeline, sample, press_obj, args):
    """Process a single sample with Pipeline - using new message format"""
    try:
        ...
        print(f"[Pipeline] Sample processing failed: {e}")
        traceback.print_exc()
        return None
...
def load_slue_dataset(json_file: str, audio_base_dir: str) -> List[Dict[str, Any]]:
    ...
    if not isinstance(data, list):
        print("Error: JSON file format should be a list")
        return []
    dataset = []
    missing = 0
    ...
    if missing > 5:
        print(f"Warning: {missing} audio files missing in total")
    print(f"Loaded {len(dataset)} valid SLUE samples")
    return dataset
...
def main():
    ...
    if args.use_pipeline:
        ...
        if pipeline:
            print("[Pipeline] Qwen-Omni KV Press Pipeline enabled")
        else:
            print("[Pipeline] Pipeline creation failed, using legacy method")
    ...
    with tqdm(total=len(samples), desc="SLUE Evaluation (Qwen2.5 KVPress)", **tqdm_kwargs) as pbar:
            ...
            try:
                if args.use_pipeline and pipeline:
                    ...
                    if result:
                        ...
                        print(f"[Pipeline] Sample {idx} processing failed, skipped")
                    pbar.update(1)
                    continue
                ...
                if input_ids is not None and use_compression:
                    ...
                    if audio_detected:
                        print(f"[KVPress] Audio token detected, enabling {ENV_PRESS_TYPE} compression")
                    else:
                        print(f"[KVPress] Audio token not detected, still enabling {ENV_PRESS_TYPE} compression")
