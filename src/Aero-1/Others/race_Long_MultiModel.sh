...
QWEN_AUTO_GPU=true    # Automatically select the GPU with the largest memory as the main GPU
...
# Show usage help
show_help() {
    echo "RACE batch evaluation script - Supports multi-model evaluation"
    echo ""
    ...
    echo "  --auto-gpu                   Automatically select the GPU with the largest memory as Qwen main GPU"
    ...
    echo "  $0 --batch --model qwen2.5 --auto-gpu   # Automatically select the GPU with largest memory"
    echo ""
...
run_single_experiment() {
    ...
    case $MODEL_TYPE in
        "phi4")
            test_script="race_test_phi4.py"
            ;;
        "aero1")
            test_script="race_test_aero_>=2min.py"
            ;;
        "kimi")
            test_script="race_test_kimi.py"
            ;;
        "qwen2.5")
            test_script="race_test_qwen2.5.py"
            ;;
        *)
