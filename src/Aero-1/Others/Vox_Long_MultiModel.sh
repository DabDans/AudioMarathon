...
SAMPLE_LIMIT=0
BATCH_MODE=false
MODEL_TYPE="qwen2.5"  # Default to Qwen2.5-Omni, options: "phi4", "aero1", "kimi", "qwen2.5"
METHODS=("random" "frame" "fast_v")
RATIOS=(0.0 0.1 0.3 0.5 0.7 0.9)
RESULTS_BASE_DIR="/data/to/your/vox/results/path"

# Show help information
show_help() {
    echo "VoxCeleb Pruning Batch Evaluation Script - Supports Multiple Models"
    echo ""
    ...
    echo "  --results-dir DIR            Specify results save directory (default: /data/to/your/vox/results/path)"
...
run_single_experiment() {
    ...
    local result_file="$RESULTS_BASE_DIR/vox_${method}_ratio${ratio}_gpu${GPU_ID}_${timestamp}.log"
    ...
    case $MODEL_TYPE in
        "phi4")
            test_script="Vox_test_phi4.py"
            ;;
        "aero1")
            test_script="Vox_test_aero.py"
            ;;
        "kimi")
            test_script="Vox_test_kimi.py"
            ;;
        "qwen2.5")
            test_script="Vox_test_qwen2.5.py"
            ;;
        *)
