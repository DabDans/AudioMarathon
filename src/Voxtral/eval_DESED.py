...
qa_json_file = "/data/to/your/DESED/path/desed_sound_event_detection_task.json"
audio_base_dir = "/data/to/your/DESED/path"
result_dir = f'/data/to/your/results/path'
...
output_file = f'{result_dir}/Voxtral_DESED_results.json'
...
def main():
    print("Starting DESED sound event detection Voxtral model evaluation...")
    ...
    with tqdm(total=len(samples), desc="Processing DESED audio samples", position=0, leave=True, **tqdm_kwargs) as pbar:
        for i, item in enumerate(samples):
            ...
            if sample_count % update_interval == 0 or sample_count == len(samples):
                current_correct = results["summary"]["correct_samples"]
                current_accuracy = current_correct / results["summary"]["total_samples"] if results["summary"]["total_samples"] > 0 else 0
    ...
    print("\n=== DESED Sound Event Detection Voxtral Model Evaluation Results Summary ===")
