...
class DART(Qwen2_5OmniThinkerTextModel):
        ...
        for idx,decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if DART_config is not None:
                    ...
                    audio_token_start_index = DART_config['audio_token_start_index']
                    audio_token_length = DART_config['audio_token_length']
                    ...
                    if idx == K and seq_length > 1:
                        ...
                        if image_token_start_index is not None and image_token_length is not None: # For image
                            # keep index
                            ...
                            cache_position = cache_position[keep_indexs]  # TODO: maybe refine exp
                            ...
                            # keep index
                            retained_audio_tokens_index = self.get_retained_audio_token(self.config, last_layer_state, any_states)

                            keep_indexs = torch.cat((torch.arange(audio_token_start_index,device=device), retained_audio_tokens_index, torch.arange(audio_token_start_index+audio_token_length,seq_length,device=device)))
                            # sort index
                            keep_indexs = keep_indexs.sort().values
                            cache_position = cache_position[keep_indexs]  # TODO: maybe refine exp
    ...
    def get_retained_image_token(self, config: Qwen2_5OmniTextConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
    ...
    def get_retained_audio_token(self, config: Qwen2_5OmniTextConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        DART_config = config.DART_config
        # K = DART_config['K']  # pruned layer
        audio_token_start_index = DART_config['audio_token_start_index']
        audio_token_length = DART_config['audio_token_length']
        ...
        pivot_audio_token = DART_config['pivot_audio_token']
        pivot_text_token = DART_config['pivot_text_token']


        reduction_ratio = DART_config['reduction_ratio']
        # TOKEN_TOPK = math.ceil(MAX_NUM_TRUNCTION / (pivot_image_token + pivot_text_token))
        TOKEN_TOPK = int(audio_token_length * (1 - reduction_ratio) / (pivot_audio_token + pivot_text_token))
        device = last_layer_state.device
        if config.text_length is not None:
            text_length = config.text_length
            audio_token_length = any_states.shape[2] - text_length
            retain_token_num_for_llava_next = min(DART_config['retain_token_num_for_llava_next'], audio_token_length - pivot_audio_token)
            TOKEN_TOPK = int(retain_token_num_for_llava_next // (pivot_audio_token + pivot_text_token))

        any_states = any_states.permute(0, 2, 1, 3).reshape(any_states.shape[0], any_states.shape[2], -1)

        k_states_audio_token = any_states[0][audio_token_start_index:audio_token_start_index + audio_token_length, :]
        k_states_query_token = any_states[0][audio_token_start_index + audio_token_length:, :]

        k_states_audio_token_L1_norm = torch.norm(k_states_audio_token, p=1, dim=-1)
        k_states_query_token_L1_norm = torch.norm(k_states_query_token, p=1, dim=-1)

        audio_indices = (k_states_audio_token_L1_norm.topk(pivot_audio_token).indices + audio_token_start_index).tolist() 
        query_indices = (k_states_query_token_L1_norm.topk(pivot_text_token).indices + audio_token_start_index + audio_token_length).tolist()
        indices_set = set(audio_indices + query_indices)

        valid_indices = set(range(audio_token_start_index, audio_token_start_index + audio_token_length)) - set(audio_indices)
        valid_indices_list = list(valid_indices)  
        for item in list(indices_set):
            valid_vectors = last_layer_state[0][valid_indices_list, :]
        
        # Time the cosine similarity calculation
            cos_sim = -torch.nn.functional.cosine_similarity(last_layer_state[0][item, :], valid_vectors, dim=-1)
        ...
        retained_audio_tokens_index = torch.tensor(list(indices_set), device=device)

        return retained_audio_tokens_index
...
class Qwen2_5OmniTalkerModel(Qwen2_5OmniPreTrainedModel):  #Hack here
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if K and (random==False):
                    ...
                    # assert retain_token_num is not None and ratio is not None, "retain_token_num and ratio cannot both be None"

                    # FASTV_image_token_length = 576 
                    # image_start = 1 
                    ...
                    if idx<K:
                        ...
                        if ratio is not None:
                            top_attention_rank_index = last_layer_attention_avg_last_tok_image.topk(int(FASTV_image_token_length * (1 - ratio))).indices + image_start  # TODO: notice here: ratio->pruning ratio
                        else:
                    ...
                    if idx<M:
                        ...
                        if ratio is not None:
                            top_attention_rank_index = last_layer_attention_avg_last_tok_audio.topk(int(FASTV_audio_token_length * (1 - ratio))).indices + audio_start  # TODO: notice here: ratio->pruning ratio
                        else:
                ...
                #Using Random
                    ...
                    retain_token_num = None
                    # assert retain_token_num is not None and ratio is not None, "retain_token_num and ratio cannot both be None"
                    # FASTV_audioaudio_token_length = 576 
                    # audio_start = 1 
                    FASTV_audio_token_length = self.config.audio_token_num if self.config.audio_token_num else None
                    audio_start = self.config.audio_token_start if self.config.audio_token_start else None
                    audio_end = audio_start + FASTV_audio_token_length
                    ...
                    if idx<M:
                        ...
                        device = hidden_states.device
                        # Get all audio token indices
                        audio_indices = torch.arange(audio_start, audio_end, device=device)
                        
                        # Frame-based pruning with flexible ratio
                        if ratio is not None:
                            # Calculate number of tokens to keep based on prune ratio
                            # ratio=0.5 means prune 50%, so keep 50% -> every 2 tokens keep 1
                            # ratio=0.3 means prune 30%, so keep 70% -> every 10 tokens keep 7
                            num_tokens_to_keep = max(1, int(FASTV_audio_token_length * (1 - ratio)))
                            
                            if num_tokens_to_keep >= FASTV_audio_token_length:
                                # Keep all tokens if ratio is very small
                                ...
                                # Convert to tensor and get corresponding audio indices
                            ...
                            # Fallback: keep specified number of tokens or all tokens
                    ...
                    if idx==M-1 and seq_length>1:
                        output_attentions=False #Notice: Because of flash attn, output_attentions is set to false
                    else:
                        output_attentions=False
                    layer_outputs = decoder_layer(
                     hidden_states,
