...
class Phi4MMDecoderLayer(nn.Module):
    def __init__(self, config: Phi4MMConfig, layer_idx: int):
        ...
        # self.self_attn = PHI4MM_ATTENTION_CLASSES["sdpa"](config, layer_idx=layer_idx)  # ! debug
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        attn_outputs, self_attn_weights, present_key_value, query_states, key_states, value_states = self.self_attn(  # ! debug
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
...
class DART(Phi4MMModel):
    ...
    def get_retained_image_token(self, config: Phi4MMConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
    ...
    def get_retained_audio_token(self, config: Phi4MMConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        DART_config = config.DART_config
        # K = DART_config['K']  # pruned layer
        audio_token_start_index = DART_config['audio_token_start_index']
        audio_token_length = DART_config['audio_token_length']
        ...
        total_cosine_time += (cosine_end - cosine_start)

        if not hasattr(self, 'cosine_time'):
            self.cosine_time = 0
            self.cosine_time += total_cosine_time
